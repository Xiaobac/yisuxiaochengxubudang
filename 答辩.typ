...
#align(center)[
  ...
  #grid(
    ...
    text(size: 10.5pt)[汪嘉桥 Raven, 高出寒 logchar, 曹昱晨 Chiaki],
    text(size: 10.5pt, fill: black.lighten(60%))[学　　校：],
    text(size: 10.5pt)[中国地质大学(武汉), 武汉大学 , 中国矿业大学],
    text(size: 10.5pt, fill: black.lighten(60%))[指　　导：],
    text(size: 10.5pt)[],
...
#table(
  ...
  [用户体验流畅度], [5], [流式 AI 推荐 ✓ \ 滚动渐变 Header ✓ \ Skeleton 骨架屏 ✓ \ loadingMore 互斥锁 ✓ \ Axios failedQueue token 刷新 ✓ \ N+1查询优化(列表页响应降至150ms) ✓ \ 详情页并行加载(TTI降低70%) ✓], [§ UX与兼容性],
  [长列表渲染优化], [3], [服务端分页(10条/页)+客户端无限滚动追加 ✓ \ onScrollToLower+loadingMore锁 ✓ \ hasMore状态管理 ✓], [§ 小程序·列表],
  [*用户体验*], [*10*], [], [],
  [视觉设计与布局], [6], [Ant Design 企业级组件 ✓ \ 深色/浅色双主题（CSS 变量 + tokens）✓ \ 统一 Icon/Skeleton/Loading/EmptyState ✓ \ 卡片圆角阴影 ✓], [§ U3X与兼容性],
  ...
  [编码规范与 README], [3], [ESLint 统一规范 ✓ \ TypeScript 全链路(消除全部18处any) ✓ \ README 含完整启动/种子/排障指南 ✓], [§ 代码质量],
  [代码复用与组件抽象], [3], [Calendar/Icon/Skeleton/AiChatWidget/FilterPanel 等 8 个共享组件 ✓ \ verifyAuth() 被29个API文件共享 ✓ \ checkPermission() 统一权限检查(5处替换) ✓ \ useTheme() 全局主题 Hook ✓], [§ 代码质量],
  [*项目创新*], [*10*], [], [],
  [提升研发效率新技术], [5], [Taro(多端编译) / Prisma(Schema驱动) / LangChain(LLM调用) / Zod(类型+校验) / next-themes(深色模式) / Swagger(自动文档) ✓], [§ 创新点详解],
  [自主 UX 创新功能], [5], [AI自然语言推荐+流式输出 / 可拖拽悬浮AI按钮 / 滚动渐变Header / 地图视图切换 / 优惠券积分兑换系统 / 城市搜索网格布局 / 搜索历史去重 / 商户数据可视化看板 ✓], [§ 创新点详解],
)

= 快速概览（Executive Summary）
...
#table(
  ...
  table.header([*维度*], [*数值*], [*说明*]),

  [后端 API 接口], [43], [覆盖认证、酒店、预订、评价、优惠券、地图、AI 推荐等 7 大模块],
  [数据库表], [14], [User、Hotel、RoomType、Booking、Review、Coupon、Location、Tag 等],
  [前端页面], [26], [Web 管理端 12 页 + 小程序用户端 14 页],
  [可复用组件], [15], [Calendar、Icon、Skeleton、FilterPanel、AiChatWidget 等],
  [代码总行数], [~12,000], [TypeScript/JavaScript/Typst，不含 node_modules],
)

== 核心性能指标

*测试环境*：MacBook Air + MySQL 8.x 本地数据库 + 1000 条酒店测试数据 + 单机测试

*性能术语说明*：P50 表示 50% 请求的响应时间，P95 表示 95% 请求的响应时间
...
#table(
  ...
  [API 平均响应时间], [< 200ms], [P50，不含 AI 推荐（流式输出无固定耗时）],
  [酒店列表接口], [~150ms], [10 条数据含 roomTypes 关联查询，N+1 优化后从 500ms 降至 150ms],
  [酒店详情页 TTI], [~250ms], [并行加载优化后从 800ms 降至 250ms（70% 提升）],
  [数据库查询延迟], [< 50ms], [单表查询 P95，含索引优化（`location_id`、`status` 复合索引）],
  [事务处理时间], [< 300ms], [预订事务含 3-7 日逐日库存 upsert，MySQL InnoDB 行锁保护],
  [并发支持], [100+ QPS], [单机测试，基于 Next.js serverless 架构可水平扩展至更高 QPS],
  [首屏加载时间], [< 1.5s], [小程序首页 P50，含 Skeleton 骨架屏优化],
)
...
#table(
  ...
  [前端框架], [Next.js 15.1（Web）、Taro 4.1.11（小程序多端）],
  [UI 组件库], [Ant Design 5.24、Taro UI],
  [后端 Runtime], [Node.js 23 + Next.js API Routes],
  [数据库], [MySQL 8.x + Prisma ORM 6.1],
  [认证鉴权], [JWT（双 Token）+ bcrypt + RBAC 权限模型],
  [AI 集成], [LangChain 0.3.19 + OpenAI GPT-4o],
  [地图服务], [腾讯地图 WebService API + 微信小程序地图组件],
  [部署方式], [Vercel（Web）+ 微信开发者工具（小程序）],
)
...
#table(
  columns: (auto, auto, 1fr),
  align: (center, center, left),
  table.header([*成员*], [*提交数*], [*负责模块与主要贡献*]),

  [Raven], [101+], [
    *项目架构与基础设施*：项目初始化、MonoRepo 结构搭建、Next.js 迁移、Prisma Schema 设计与种子数据 \
    *PC 端全栈开发*：管理员后台（审核页、欢迎页、登录页）、商户后台（房型表单、数据看板）、完整 PC 前端页面搭建 \
    *小程序核心功能*：Taro 项目创建、首页搜索与 Banner、酒店详情页（Swiper 轮播 / 房型排序 / 剩余房量）、订单详情、地图定位、收藏功能、个人中心 \
    *日历组件*：无限滚动、房型价格联动、农历显示、震动反馈 \
    *UI 与体验*：深色模式全局适配（CSS 变量 + tokens）、Phosphor Icons 引入、错落渐显动画、弹窗动画、Skeleton 骨架屏 \
    *安全与质量*：注册权限漏洞修复、Zod v4 兼容、参数校验加固、代码重构与冲突清理 \
    *文档*：答辩文档撰写与持续维护
  ],

  [Chuhan Gao], [61], [
    *后端 API 开发*：酒店 CRUD、用户 JWT 鉴权、预订接口、房间管理、标签与位置、Swagger 自动文档 \
    *AI 智能推荐*：LangChain + OpenAI 集成、多轮对话优化、流式输出 \
    *评价系统*：评论区基本功能、评分系统（移动端 + 管理端 + 商户端三端适配） \
    *优惠券与积分*：积分后端实现、积分兑换优惠券、管理端优惠券页面、移动端适配 \
    *腾讯地图*：商户端地图接入、移动端地图适配、地图修复 \
    *列表与搜索*：酒店列表分页渲染优化、城市搜索与位置树状结构、首页 searchParams 同步、storageSync 同步 \
    *其他*：订单流程优化、Token 刷新机制、管理员功能完善、商户页面后端适配、数据文件更新
  ],

  [Chiaki], [11], [
    *移动端 AI 助手*：小程序端 AI 聊天组件开发 \
    *UI 美化*：酒店详情页美化、评论区样式设计 \
    *优惠券页面*：移动端优惠券展示页开发 \
    *日历与列表优化*：日历交互优化、列表渲染改进 \
    *多端适配*：QQ 小程序与支付宝小程序初步适配 \
    *国际化*：部分页面多语言切换适配
  ],
)
...
本项目在技术选型与工程实践上有以下核心亮点（详见第 5 章"创新点详解"）：

*① 事务性防超卖机制* — 数据库事务 + 原子 upsert 逐日库存，从存储层杜绝超卖。

*② 大语言模型两阶段推荐* — LLM 分离意图提取（temperature=0）与文案生成（temperature=0.7），结构化输出 + 流式推荐。

*③ 精细化 RBAC 双层鉴权* — JWT 粗粒度 + Role→Permission 细粒度，状态变更原子写审计日志。

*④ 多端统一架构* — Web 端 + 小程序端复用同一套 Next.js API Routes，Taro 一套代码多端编译。

*⑤ 全栈类型安全* — Prisma 生成数据库类型，前后端共享 TypeScript 定义，LangChain 与 Zod 共用 Schema。

*⑥ 长列表渲染优化* — 服务端分页（10条/页）+ 客户端无限滚动，首屏仅加载 10 条数据，渲染性能线性可控。

*⑦ 工程化规范* — JSDoc 自动生成 Swagger 文档，文件上传安全化，Prisma 单例防连接泄漏。
...
#table(
  ...
  [②], [数据库全貌], [#flink("prisma/schema.prisma#L182-L196")], [14 个数据模型的字段、关系、约束（重点看 RoomAvailability 复合唯一约束）],
  [③], [接口鉴权机制], [#flink("app/api/utils/auth.ts#L17-L30")], [`verifyAuth()` 如何解析 JWT，所有受保护接口的统一调用方式],
  [④], [Web 前端请求层], [#flink("app/lib/request.ts#L47-L145")], [Axios 拦截器、自动 Token 刷新、401 failedQueue 并发控制],
  [⑤], [小程序请求层], [#flink("trip_front_end_taro/src/services/request.js")], [`Taro.request` 封装、Token 注入、网络错误提示],
  [⑥], [数据库单例], [#flink("app/lib/prisma.ts#L4-L12")], [PrismaClient globalThis 单例，防止热更新连接泄漏],
)
...
#table(
  ...
  [职员管理], [staff 页面 CRUD 职员（邮箱/密码/姓名/手机）→ 仅商户角色可见], [User 记录（role=STAFF，merchantId 关联当前商户）],
  [个人信息], [profile 页面查看个人资料 → 编辑姓名手机 → 修改密码], [User 更新 + localStorage 同步],
  [优惠券管理], [coupons 页面 CRUD 优惠券（代码/名称/折扣/积分/有效期）→ 仅商户角色可见], [Coupon 增删改],
)
...
#table(
  ...
  [优惠券管理], [coupons 页面 CRUD 优惠券（代码/名称/折扣/积分/有效期）], [Coupon 增删改，供用户端领取和预订抵扣],
)
...
- *服务层*：对 API 调用进行封装，Web 侧用 Axios（#flink("app/lib/request.ts#L6-L9")），小程序侧用 `Taro.request`；
- *接口层*：Next.js API Routes 处理 HTTP 请求，`verifyAuth()`（#flink("app/api/utils/auth.ts#L17-L30")）统一校验 JWT，细粒度权限查询 Permission 表；
- *数据访问层*：Prisma Client 单例（#flink("app/lib/prisma.ts#L4-L12")）提供类型安全的 ORM 操作，支持 `$transaction`；
...
│   │   ├── users/coupons/route.ts       # GET 当前用户已领取优惠券列表
│   │   ├── locations/
...
│   │   ├── comments/
│   │   │   ├── route.ts             # GET 评论列表 / POST 创建评论
│   │   │   └── [id]/route.ts        # GET 详情 / PUT 修改 / DELETE 删除
│   │   ├── room-availability/
│   │   │   ├── route.tsx            # GET / POST 库存批量操作
│   │   │   └── [id]/route.tsx       # 单条库存 GET / PUT
...
│   │   ├── hotels/
│   │   │   ├── page.tsx              # 酒店管理页（上下线操作）
│   │   │   └── components/
│   │   │       ├── HotelDetailsDrawer.tsx    # 酒店详情抽屉组件
│   │   │       ├── HotelCommentsDrawer.tsx   # 酒店评论抽屉组件
│   │   │       └── useHotelTableColumns.tsx  # 表格列定义 Hook
│   │   ├── coupons/page.tsx          # 优惠券管理页（CRUD + 积分设置）
│   │   ├── locations/page.tsx        # 城市管理页
│   │   └── tags/page.tsx             # 标签管理页
│   ├── merchant/                     # 商户页面（需 MERCHANT/STAFF 角色）
│   │   ├── layout.tsx                # 商户侧边栏布局（按角色动态菜单）
...
│   │   ├── staff/page.tsx            # 职员管理（仅 MERCHANT 可见）
│   │   ├── profile/page.tsx          # 个人信息（编辑资料 + 修改密码）
│   │   └── coupons/page.tsx          # 优惠券管理（仅 MERCHANT 可见）
│   ├── auth/
...
│   │   ├── comment.ts                # 评论相关 API 封装
│   │   ├── coupon.ts                 # 优惠券相关 API 封装（getCoupons / createCoupon / claimCoupon）
│   │   ├── staff.ts                  # 职员管理 + 个人信息 API 封装（getMyStaff / createStaff / getProfile / changePassword）
...
│   │   ├── auth-storage.ts           # 认证状态统一管理（Token/User 读写 + SSR 守卫）
│   │   ├── error-handler.ts          # 统一错误处理工具（Prisma/Zod/通用异常）
│   │   └── swagger.ts                # Swagger 规范配置
│   ├── constants/index.ts             # 全局常量（API 端点、状态码、角色名、存储键）
...
└── trip_front_end_taro/              # 小程序端（独立子项目）
    ...
    │   │   ├── myCoupons/index.jsx   # 我的优惠券（已领取券列表 + 状态管理）
    ...
    │   │   ├── review.js             # 评价相关 API
    │   │   ├── location.js          # 城市相关 API
    │   │   ├── tag.js               # 标签相关 API
    │   │   ├── coupon.js            # 优惠券相关 API（getUserCoupons / claimCoupon）
...
#table(
  ...
  [#flink("trip_front_end_taro/src/pages/myCoupons/index.jsx")], [页面], [我的优惠券：展示已领取券，区分可用/已使用/已过期三种状态，可用券提供"去使用"跳转],
...
后端按业务域划分，每个模块对应 #flink("app/api/") 下的独立路由组。模块间通过 Prisma Client 共享数据库连接（#flink("app/lib/prisma.ts#L8-L11") 单例模式），通过 `verifyAuth()`（#flink("app/api/utils/auth.ts#L17-L30")）共享鉴权工具，业务逻辑互不耦合。
...
#table(
  ...
  [评论模块], [`/api/comments`], [酒店评论 CRUD（支持按 hotelId/userId 筛选），创建/删除自动调用 `updateHotelScore()` 重算酒店评分],
...
登录流程涉及 #flink("app/api/auth/login/route.tsx#L127")（bcrypt 密码校验）、#flink("app/api/auth/login/route.tsx#L145-L151")（JWT 双 Token 签发）、#flink("app/api/utils/auth.ts")、前端 #flink("app/services/auth.ts") 和 #flink("app/lib/request.ts")。系统采用双 Token 机制，Access Token 有效期 1 小时用于接口认证，Refresh Token 有效期 7 天用于无感刷新。
...
每个受保护的 API Route 在业务逻辑前统一调用 `verifyAuth(request)`（#flink("app/api/utils/auth.ts#L17-L30")）。该函数提取 `Authorization` Header 中的 Bearer Token，使用 `jwt.verify()` 解码并返回 `DecodedUser`，失败时返回含 HTTP 状态码的错误对象。调用方直接解构结果进行早返回，代码模式统一整洁。

对于需要精细权限的操作（如审核通过/拒绝、删除酒店），在 JWT 角色校验通过后，还需额外查询 `role.rolePermission` 列表，检查是否包含 `HOTEL_AUDIT` 或 `HOTEL_DELETE` 权限名称。

=== 在线预订事务流程

预订流程是系统最复杂的业务链路，涉及文件：小程序 #flink("trip_front_end_taro/src/components/BookingConfirm/index.jsx") → #flink("trip_front_end_taro/src/services/booking.js") → #flink("app/api/bookings/route.tsx#L208-L337")（事务防超卖核心逻辑）。核心挑战是在高并发场景下防止超卖，通过数据库事务解决：
...
*核心文件*：#flink("app/api/utils/permissions.ts#L7-L71")（`PermissionCache` 类）、#flink("app/api/utils/permissions.ts#L110-L142")（批量权限检查 API）

*业务价值*：解决商户管理端频繁权限查询的性能瓶颈。管理员在酒店审核、用户管理等页面操作时，每个 API 请求都需要校验细粒度权限（HOTEL_AUDIT、USER_MANAGE 等），未优化前每次查询需 35-50ms，高频操作下响应延迟明显。

*做了什么*：实现 15 分钟 TTL 内存缓存机制，将用户权限集合缓存在应用内存中，避免每次权限检查都进行深层数据库查询（`role → rolePermission → permission` 三表关联）。

*怎么做的*：

`PermissionCache` 单例类（#flink("app/api/utils/permissions.ts#L7-L71")）管理缓存层，使用 `Map` + `Set` 数据结构存储。`checkPermission(userId, permissionName)` 函数实现三步查询逻辑：① 尝试从缓存获取权限集合；② 缓存未命中时通过 Prisma 查询数据库并写入缓存；③ 使用 `Set.has()` 高效检查权限（O(1) 时间复杂度）。

数据库查询优化：仅 select 必需字段（`permission.name`），减少数据传输量。缓存过期策略采用被动检查模式——每次 `get()` 时比对时间戳，超过 15 分钟自动失效并返回 `null`。提供 `clearPermissionCache(userId)` 接口，在用户角色变更时主动清除缓存，保证数据一致性。

批量权限检查 API：`checkAnyPermission()`（OR 逻辑）和 `checkAllPermissions()`（AND 逻辑），均复用缓存层。

*性能收益*（本地测试环境）：权限检查响应时间从 35-50ms 降至 < 2ms（缓存命中场景），缓存命中率约 90%；管理后台操作流畅度明显提升，特别是酒店列表批量操作场景。
...
- 后端接口：#flink("app/api/bookings/route.tsx#L208-L337")（POST 方法，核心事务逻辑：逐日检查库存并原子递增 booked）
...
后端 POST 接口先调用 `verifyAuth()` 确认用户已登录，再校验日期合法性，然后开启 `prisma.$transaction`。事务内采用*批量查询 + 逐日原子更新*的优化策略：

+ *批量查询阶段*（3 次数据库操作）：① 一次性 `findMany` 查询入住区间内所有日期的 `RoomAvailability` 记录；② 对缺失日期执行 `createMany({ skipDuplicates: true })` 批量初始化；③ 再次 `findMany` 获取完整可用性数据。在内存中遍历验证每日状态（`isClosed`、`booked >= quota`），累加每日价格计算总价——该阶段将原先 N×2 次查询优化为固定 3 次。

+ *逐日原子更新阶段*（N 次数据库操作）：对每一天使用条件 `updateMany({ where: { booked: { lt: quota } }, data: { booked: { increment: 1 } } })` 原子递增已订数，保证并发安全。如 `updated.count === 0` 则说明被其他事务抢占，抛错回滚。

综合来看，7 晚预订从原先 21 次数据库操作（每天 upsert + findUnique + updateMany）降至 10 次（3 次批量 + 7 次原子更新），减少约 52% 的数据库交互。MySQL InnoDB 行锁在事务内保护同一日期的并发写操作互斥，任意步骤失败则所有 `booked` 变更一并回滚，从数据库层彻底消除超卖。
...
*第一阶段——结构化意图提取*：使用 LangChain 的 `model.withStructuredOutput(ZodSchema)` 以温度 0（确定性输出）调用模型，从最近 3 条对话消息中提取结构化搜索条件（目的地、价格区间、关键词、设施、排序方式）。Zod Schema（#flink("app/api/ai/recommend/route.ts#L9-L16")）同时约束输出结构并与 TypeScript 类型同步。
...
*Web 端（#flink("app/lib/request.ts")）*：以 `axios.create({ baseURL: '/api', timeout: 10000 })` 创建实例，挂载两个拦截器。请求拦截器通过 `authStorage.getToken()` 统一读取 Token（该工具自动处理 SSR 环境守卫），写入 `Authorization: Bearer <token>` Header。响应拦截器的成功回调直接 `return response.data`，让调用方无需每次解包；失败回调遇到 401 时，先尝试用 `refreshToken` 调用 `/api/auth/refresh` 无感换取新 `accessToken`——通过 `isRefreshing` 布尔锁 + `failedQueue` 队列保证并发安全（多个请求同时 401 时只发一次 refresh，其余请求排队等待新 Token 后自动重发）；refresh 成功后调用 `authStorage.saveAuth(newToken, newRefreshToken, user)` 统一写入存储，原始请求自动重试；refresh 失败则调用 `authStorage.clearAuth()` 统一清除认证数据并 `window.location.href = '/auth/login'` 跳转登录页。最终导出泛型化的 `get<T>/post<T>/put<T>/del<T>` 方法，调用方可直接标注返回类型。
...
=== 认证状态统一管理 #flink("app/lib/auth-storage.ts")

*做了什么*：封装所有 localStorage 认证相关操作，消除代码重复，提供类型安全的 API，实现认证状态的集中式管理。

*怎么做的*：

导出 `authStorage` 对象，包含完整的认证状态管理方法：`getToken()` / `setToken(token)` 读写访问令牌，`getRefreshToken()` / `setRefreshToken(token)` 管理刷新令牌，`getUser()` / `setUser(user)` 处理用户信息的 JSON 序列化与反序列化，`saveAuth(token, refreshToken, user)` 批量写入认证三元组，`clearAuth()` 一键清除所有认证数据。

所有方法内部统一处理 SSR 环境守卫（`typeof window !== 'undefined'`），读取操作在服务端渲染时自动返回 `null`，避免 `localStorage is not defined` 错误。用户对象的 `getUser()` 自动捕获 JSON 解析异常并返回 `null`，保证存储损坏时不抛异常。

辅助校验方法：`isAuthenticated()` 检查 Token 是否存在（返回布尔值），`hasRole(role)` 检查当前用户是否拥有指定角色（支持大小写不敏感比对）。`STORAGE_KEYS` 常量从 `app/constants/index.ts` 导入，保证键名全局统一。

*使用场景*：

- `app/lib/request.ts` — 请求拦截器通过 `authStorage.getToken()` 读取 Token，响应拦截器通过 `authStorage.saveAuth()` 写入刷新后的 Token；
- `app/services/auth.ts` — 登录成功后调用 `authStorage.saveAuth(accessToken, refreshToken, user)` 一次性写入所有认证数据，登出时调用 `authStorage.clearAuth()` 清除；
- `app/hooks/useAuth.ts` — 前端路由保护 Hook 通过 `authStorage.isAuthenticated()` 检查登录态，通过 `authStorage.getUser()` 获取当前用户；
- 总计约 8 个文件共享该工具，修改一处即全局生效，彻底消除 `localStorage.getItem('token')` 等原始调用散布在代码各处的问题。

=== 完整调用示例：Taro 小程序加载酒店列表

以下展示从前端发起请求到后端返回数据的完整数据流转，演示三端共用 API 架构的实际运作机制。

*场景*：用户在小程序酒店列表页搜索"上海"地区的酒店，系统需从后端获取符合条件的数据并渲染。

*① 前端页面层*（#flink("trip_front_end_taro/src/pages/hotelList/index.jsx#L238-L243")）

页面组件调用 Service 层的业务函数，传入筛选参数：

```javascript
const loadHotels = async (params = {}) => {
  const res = await getHotels({
    locationId: params.locationId,  // 城市 ID
    keyword: params.keyword,        // 搜索关键词
    type: params.type,              // 酒店类型
    page: 1,
  });
  setHotels(res.data);  // res.data 已是解析后的酒店数组
};
```

*② Service 业务层*（#flink("trip_front_end_taro/src/services/hotel.js#L23-L55")）

Service 层负责参数清洗和 API 路径映射，过滤 `null/undefined` 值后调用请求封装：
...
*③ 请求封装层*（#flink("trip_front_end_taro/src/services/request.js#L22-L53")）

请求封装层自动从本地存储读取 Token，注入 `Authorization` Header，发起 HTTP 请求：
...
function request(url, options = {}) {
  ...
  return Taro.request({
    url: BASE_URL + url,  // http://localhost:3000/api/hotels?locationId=1&keyword=上海
    ...
    return res.data;  // 自动解包，返回 { success: true, data: [...] }
  });
}
```

*④ Next.js API Routes*（#flink("app/api/hotels/route.tsx#L167-L240")）

后端 API 解析查询参数，构建 Prisma 查询条件，查询数据库并返回 JSON：
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    success: true,
    data: hotels,
    pagination: { page, limit, total: await prisma.hotel.count({ where }) },
  });
}
```
...
后端返回的 JSON 响应自动被请求封装层解包，业务代码直接获得 `data` 字段：
  ...
  "data": [
    {
      "id": 1,
      "nameZh": "上海浦东希尔顿酒店",
      "address": "浦东新区世纪大道XXX号",
      "starRating": 5,
      "location": { "id": 1, "name": "上海" },
      "roomTypes": [
        { "id": 1, "name": "豪华大床房", "price": 899, "stock": 10 }
      ],
      "hotelTags": [
        { "tag": { "id": 1, "name": "商务出行" } }
      ]
...
*关键优势*：整个流程中，Token 自动注入、响应解包、401 跳转登录、网络错误提示均由请求封装层统一处理，页面组件和 Service 层的业务代码零冗余。Web 端的 #flink("app/lib/request.ts") 采用相同架构，但使用 Axios 拦截器实现更复杂的 Token 自动刷新机制（#flink("app/lib/request.ts#L86-L133")），多个并发 401 请求通过 `failedQueue` 队列合并为一次刷新操作，避免重复调用 `/auth/refresh` 接口。
...
- 小程序主题工具函数：#flink("trip_front_end_taro/src/utils/theme.js#L21-L58")（`TOKENS` 深色/浅色主题色定义）、`resolveTheme` / `getThemeCssVars` / `applyNativeTheme`
...
- 优惠券管理页：#flink("app/admin/coupons/page.tsx")
- 数据服务层：#flink("app/services/coupon.ts")（`getCoupons`、`createCoupon`、`updateCoupon`、`deleteCoupon`）
- 后端接口：#flink("app/api/coupons/route.ts")（GET/POST）、#flink("app/api/coupons/[id]/route.ts")（PUT/DELETE）

=== 做了什么

管理员进入优惠券管理页后，以表格形式查看系统全部优惠券（代码、名称、折扣金额、最低消费、兑换所需积分、有效期），可执行新增、编辑和删除操作。

=== 怎么做的

页面使用 `useAdminAuth` Hook 校验当前用户角色，非 ADMIN 自动重定向。表格以 Ant Design `<Table>` 渲染优惠券列表，列包括：优惠券代码（`<Tag color="blue">`）、名称、折扣金额与最低消费（组合显示"减X元（满Y可用）"）、兑换积分、有效期范围、动态状态标签（根据当前日期判定"生效中"/"未开始"/"已过期"），以及编辑/删除操作按钮。

新增和编辑共用同一个 `<Modal>` + `<Form>` 弹窗（通过 `editingCoupon` 状态区分模式），使用 `Form.useForm()` 管理表单状态。表单字段包括：优惠券代码（`Input`，必填）、名称（`Input`，必填）、描述（`TextArea`，可选）、折扣金额（`InputNumber`，必填）、最低消费（`InputNumber`，可选）、兑换所需积分（`InputNumber`，可选，默认 0 表示免费领取）、有效期（`DatePicker.RangePicker`，必填）。提交时通过 `form.validateFields()` 校验后调用 `createCoupon` 或 `updateCoupon`，成功后重新拉取列表。删除操作通过 `<Popconfirm>` 二次确认后调用 `deleteCoupon`，成功后乐观更新本地 state。反馈统一使用 `App.useApp()` 的 `message` 组件。

== 商户职员管理实现

=== 相关文件

- 职员管理页：#flink("app/merchant/staff/page.tsx")
- 数据服务层：#flink("app/services/staff.ts")（`getMyStaff`、`createStaff`、`updateStaff`、`deleteStaff`）
- 后端接口：#flink("app/api/users/route.tsx")（GET `?myStaff=true`）、#flink("app/api/users/[id]/route.tsx")（PUT/DELETE，商户管理职员权限）
- 注册接口：#flink("app/api/auth/register/route.tsx")（POST，`role=staff` + `merchantId` 创建职员）

=== 做了什么

商户可在侧边栏"职员管理"页面中查看、添加、编辑和删除所属职员账号。职员角色（STAFF）登录后共享同一套商户管理系统（仪表盘、酒店、日历、预订），但侧边栏不显示"职员管理"和"优惠券管理"两项菜单，实现基于角色的功能隔离。

=== 怎么做的

*后端权限扩展*：GET `/api/users` 新增 `?myStaff=true` 查询路径，校验调用者角色为 MERCHANT 后返回 `merchantId` 等于当前用户 ID 的所有职员记录。PUT/DELETE `/api/users/[id]` 在原有"本人或管理员"权限基础上，新增商户管理自身职员的判断逻辑——通过 `targetUser.merchantId === currentUserId` 确认归属关系后放行操作，无需在权限表中额外配置。

*前端实现*：页面使用 `getStoredUser()` 获取当前用户角色，非 MERCHANT 展示 `<Empty description="无权访问">` 占位。表格以 Ant Design `<Table>` 渲染职员列表（姓名、邮箱、手机、创建时间、操作列）。添加职员时通过 `<Modal>` + `<Form>` 收集邮箱、密码、姓名、手机字段，调用 `createStaff()` 封装的注册接口（自动附加 `role: 'staff'` 和 `merchantId`）。编辑仅允许修改姓名和手机，删除通过 `<Popconfirm>` 二次确认。

*菜单隔离*：`layout.tsx` 在构建 `menuItems` 时通过 `isMerchant` 判断动态展开"职员管理"和"优惠券管理"菜单项（`...(isMerchant ? [...] : [])`），STAFF 角色登录后仅看到仪表盘、酒店管理、房间日历、预订管理四个入口。

== 商户个人信息实现

=== 相关文件

- 个人信息页：#flink("app/merchant/profile/page.tsx")
- 数据服务层：#flink("app/services/staff.ts")（`getProfile`、`updateProfile`、`changePassword`）
- 后端接口：#flink("app/api/users/profile/route.tsx")（GET 当前用户资料）、#flink("app/api/users/[id]/route.tsx")（PUT 更新资料/密码）

=== 做了什么

商户和职员均可通过右上角头像下拉菜单的"个人信息"选项进入个人资料页，查看邮箱、姓名、手机、角色、积分、注册时间等信息，并支持编辑基本信息和修改密码。

=== 怎么做的

页面加载时调用 `getProfile()` 从服务端获取最新用户数据，以 `<Card>` + `<Descriptions>` 展示。编辑信息和修改密码分别使用独立的 `<Modal>` + `<Form>`，各自绑定 `Form.useForm()` 实例。

*编辑信息*：Modal 打开时通过 `form.setFieldsValue()` 回填当前姓名和手机，提交后调用 `updateProfile(userId, { name, phone })` 更新，成功后同步更新页面 state 和 `authStorage.setUser()`（保证 Header 中的用户名实时刷新）。

*修改密码*：后端 PUT `/api/users/[id]` 新增 `oldPassword` + `newPassword` 字段支持，仅允许本人操作（`isSelf` 校验）。服务端通过 `bcrypt.compare()` 验证旧密码正确性，不匹配返回 400；验证通过后 `bcrypt.hash(newPassword, 10)` 加密存储。前端表单包含旧密码、新密码、确认密码三个字段，确认密码通过 `dependencies + validator` 实时校验一致性。

== 商户优惠券管理实现

=== 相关文件

- 优惠券管理页：#flink("app/merchant/coupons/page.tsx")
- 数据服务层：#flink("app/services/coupon.ts")（`getCoupons`、`createCoupon`、`updateCoupon`、`deleteCoupon`）
- 后端接口：#flink("app/api/coupons/route.ts")（GET/POST）、#flink("app/api/coupons/[id]/route.ts")（PUT/DELETE）

=== 做了什么

商户可在侧边栏"优惠券管理"页面中管理系统优惠券，与管理员端共享同一套后端接口和数据服务层，仅前端入口和角色校验不同。

=== 怎么做的

页面结构与管理员优惠券页保持一致（Ant Design `<Table>` + `<Modal>` + `<Form>` 模式），角色校验通过 `getStoredUser()` 判断是否为 MERCHANT，非商户展示 `<Empty>` 占位。表格列新增"状态"列，根据当前日期与 `validFrom`/`validTo` 对比动态显示 `<Tag color="green">生效中</Tag>`、`<Tag color="orange">未开始</Tag>` 或 `<Tag color="default">已过期</Tag>`。有效期输入使用 `<DatePicker.RangePicker>` 替代原生日期选择器，提升交互体验。
...
#table(
  ...
  [`searchHistory`], [`[]`], [`Array<String>`], [Taro Storage 持久化的搜索历史（最多10条）],
...
首页和列表页均提供城市搜索功能，共用同一套交互逻辑与 A-Z 拼音索引选择器。

*A-Z 拼音分组机制*

自建工具模块 #flink("trip_front_end_taro/src/utils/pinyinMap.js")，维护 `CHAR_TO_INITIAL` 映射表（覆盖 70+ 常用汉字首字 → 大写拼音字母），`getInitial(name)` 取城市名首字查表返回字母（英文直接 `toUpperCase`，未匹配返回 `#`）。`groupByInitial(cities)` 遍历城市列表按首字母分桶，再按字母排序输出 `[{letter, cities}]` 数组，供前端渲染分组视图。

*城市选择器 UI*

弹出层顶部提供"国内/海外"双 Tab 切换（`cityTab`），根据 `location.type` 过滤城市；下方设搜索框实时前端过滤（`name.includes(keyword)`）。城市以 3 列网格按字母分组展示，每组显示字母标题行。弹窗右侧渲染字母索引栏，点击字母通过 `ScrollView.scrollIntoView` 快速滚动至对应分组，实现通讯录式检索体验。左上角"重置"按钮清除城市选择。

*搜索条件互斥逻辑*

关键词搜索与城市筛选设计为互斥关系，避免产生冲突的查询结果：
- *输入关键词* → 自动清除已选城市（`setSelectedLocation(null)`），移除 `locationId` 参数，以关键词全局搜索
- *选择城市* → 自动清空搜索关键词（`setLocalSearchKeyword('')`），移除 `keyword` 参数，按城市定向筛选
- *清空搜索框* → 300ms 防抖后自动移除 `keyword` 并重新加载全部结果，无需手动按回车
- *重置城市* → 同时清空关键词和城市，恢复无筛选状态

首页与列表页通过 `Taro.setStorageSync('selectedCitySync')` 同步城市选择状态，保证跨页面一致性。
...
#figure(
  diagram(
    ...
    node((0,0), [用户点击城市选择按钮],
      fill: rgb("#1e3a5f"), corner-radius: 3pt, width: 130pt),
    node((0,1), [打开城市选择器 Modal\n国内/海外 Tab 切换],
      fill: rgb("#1e3a5f"), corner-radius: 3pt, width: 130pt),
    node((0,2), [GET /api/locations\n按 type 筛选],
      fill: rgb("#3d2b00"), corner-radius: 3pt, width: 130pt),
    node((0,3), [groupByInitial 拼音分组\n渲染 A-Z 索引 + 网格],
      fill: rgb("#1a4a2e"), corner-radius: 3pt, width: 130pt),
    node((0,4), [用户点击右侧字母索引],
      fill: rgb("#1e3a5f"), corner-radius: 3pt, width: 130pt),
    node((0,5), [ScrollView 滚动至\n对应字母分组],
      fill: rgb("#1a4a2e"), corner-radius: 3pt, width: 130pt),
    node((0,6), [用户点击某城市],
      fill: rgb("#1e3a5f"), corner-radius: 3pt, width: 130pt),
    node((0,7), [互斥清除：清空关键词\n更新 selectedLocation],
    ...
    edge((0,2), (0,3), "->", stroke: white.darken(30%)),
    edge((0,3), (0,4), "->", stroke: white.darken(30%),
      label: text(size: 7pt)[索引栏],
      label-side: left),
    edge((0,4), (0,5), "->", stroke: white.darken(30%)),
    edge((0,5), (0,6), "->", stroke: white.darken(30%)),
    edge((0,6), (0,7), "->", stroke: white.darken(30%)),
    edge((0,7), (0,8), "->", stroke: white.darken(30%)),
  ),
  caption: [城市搜索交互流程图（含 A-Z 索引与互斥逻辑）]
)
...
详情页需加载四类数据：酒店基本信息、房型库存价格、收藏状态、评价列表。早期实现中四个请求串行执行，总耗时约 800ms（每个请求约 200ms）。优化后通过 `Promise.all` 并行发起，总耗时降至约 250ms（取决于最慢的单个请求）：
...
const [hotelRes, roomTypesRes, favoriteRes, commentList] = await Promise.all([
  getHotelById(hotelId),                      // 酒店详情
  getHotelRoomTypes(hotelId, startDate, endDate),  // 房型与库存
  storage.isAuthenticated()                   // 收藏状态（仅登录用户）
    ? checkFavorite(hotelId).catch(() => null)
    : Promise.resolve(null),
  getHotelCommentsCombined(hotelId),          // 评价列表
]);
```

此优化直接缩短首屏可交互时间（TTI）约 70%，提升用户感知速度，同时通过 `.catch(() => null)` 确保单个接口失败不影响其他数据展示（收藏状态查询失败时降级为"未收藏"）。
...
- 组件（430 行）：#flink("trip_front_end_taro/src/components/Calendar/index.jsx#L16")（mode 属性定义：'range' 或 'single'）、#flink("trip_front_end_taro/src/components/Calendar/index.jsx#L124-L156")（mode 模式切换逻辑）
...
日历通过 `mode` 属性（#flink("trip_front_end_taro/src/components/Calendar/index.jsx#L16")）支持两种选择模式，`handleDayClick` 函数（#flink("trip_front_end_taro/src/components/Calendar/index.jsx#L124-L156")）根据模式走不同分支：

#text(size: 8.5pt, fill: black.lighten(55%))[来源：#flink("trip_front_end_taro/src/components/Calendar/index.jsx#L124-L156")]
```js
if (mode === 'single') {
  // 单日期模式（钟点房）：选中后等待用户点"确定"
  setTempStart(dayStr)
  setTempEnd('')
} else {
  // 区间模式（普通住宿）
  if (!tempStart || tempEnd) {
    ...
    if (dayStr === tempStart) return  // 不允许零晚预订
    // 自动确定入住/离店顺序
    const firstDay = dayjs(tempStart)
    if (secondDay.isAfter(firstDay, 'day')) {
      setTempStart(tempStart); setTempEnd(dayStr)
    } else {
      setTempStart(dayStr); setTempEnd(tempStart)  // 自动交换
    }
  }
}
```

首页住宿类型 Tab 切换时自动设置日历模式：钟点房 Tab 传入 `mode='single'`，其余 Tab 传入 `mode='range'`，实现 Tab 联动日历行为。
...
*酒店卡片信息*：每张酒店卡片展示酒店中文名（`nameZh`）、用户综合评分（`score`，由历史评价汇总）、地址、以及从后端返回的 `roomTypes` 数组计算得出的最低房型价格，满足题目"每条酒店可展示名称/评分/地址/价格"的要求。

*N+1 查询优化*：早期实现中，列表页对每个酒店单独调用 `getHotelMinPrice(hotelId)` 获取最低价，导致 10 条列表产生 11 次 SQL 查询（1 次查酒店 + 10 次查价格）。优化后，后端 `GET /api/hotels` 通过 Prisma `include: { roomTypes: { select: { price: true } } }` 一次性返回全部房型价格，前端直接在内存中计算 `Math.min(...roomTypes.map(rt => rt.price))`，查询次数降至 1 次，响应延迟从约 500ms 降至约 150ms（10 条数据）。
...
=== 相关文件
- #flink("trip_front_end_taro/src/pages/favoriteList/index.jsx")
- #flink("trip_front_end_taro/src/services/favorite.js")（`getFavorites`）

=== 做了什么
展示当前用户收藏的所有酒店卡片列表，支持取消收藏操作，点击卡片跳转酒店详情页。

=== 怎么做的
页面挂载时并发调用 `getFavoriteFolders()` 和 `getMyFavorites()`（#flink("trip_front_end_taro/src/pages/favoriteList/index.jsx#L35-L38")），支持收藏夹分组管理。取消收藏通过二次确认弹窗 + `removeFavorite(hotelId)` 发送 `DELETE /api/favorites/{hotelId}`，成功后乐观更新本地 state（#flink("trip_front_end_taro/src/pages/favoriteList/index.jsx#L108-L130")）。空列表展示 `EmptyState`，支持下拉刷新（`usePullDownRefresh`）和批量操作模式。

#text(size: 8.5pt, fill: black.lighten(55%))[来源：#flink("trip_front_end_taro/src/pages/favoriteList/index.jsx#L108-L130")]
```jsx
// 取消收藏——二次确认 + 乐观更新
const handleRemoveFavorite = (hotelId, e) => {
  e.stopPropagation();
  Taro.showModal({
    title: '取消收藏', content: '确定要取消收藏这家酒店吗？',
    success: async (res) => {
      if (res.confirm) {
...
=== 相关文件
- #flink("trip_front_end_taro/src/pages/reviewList/index.jsx")
- #flink("trip_front_end_taro/src/services/review.js")（`getMyReviews`）

=== 做了什么
展示当前用户提交的所有酒店评价，每条评价显示酒店名称、评分星级、评价内容、提交时间，支持删除评价操作。

=== 怎么做的
页面挂载前先通过 `storage.isAuthenticated()` 校验登录态，未登录自动跳转登录页（#flink("trip_front_end_taro/src/pages/reviewList/index.jsx#L21-L26")）。评价卡片展示 `'★'.repeat(rating) + '☆'.repeat(5-rating)` 五星评分，支持行内编辑（#flink("trip_front_end_taro/src/pages/reviewList/index.jsx#L48-L58") `handleEdit` 切换编辑态）和删除操作。删除通过二次确认弹窗 + `deleteReview(reviewId)` 乐观更新列表。

#text(size: 8.5pt, fill: black.lighten(55%))[来源：#flink("trip_front_end_taro/src/pages/reviewList/index.jsx#L77-L93")]
```jsx
// 删除评价——二次确认 + 乐观更新
const handleDelete = (reviewId) => {
  Taro.showModal({
    title: '提示', content: '确定要删除这条评价吗？',
    success: async (res) => {
      if (res.confirm) {
...
- 后端接口：#flink("app/api/coupons/[id]/claim/route.ts#L37-L60")（积分兑换事务：检查余额 → 扣减积分 → 发放优惠券）、#flink("app/api/user/coupons/route.ts")

=== 做了什么

实现完整的优惠券积分兑换体系，包括：① 优惠券展示与领取/兑换；② 预订时优惠券选择与抵扣；③ 积分累积机制；④ 优惠券生命周期管理。

*业务流程*：
- *积分来源*：用户完成订单后自动获得积分奖励（当前规则：每消费 1 元获得 1 积分，在 `Booking.status` 变更为 `completed` 时累加到 `User.points`）
- *领取/兑换*：优惠券页展示所有可用券，免费券（points = 0）直接领取，积分券（points > 0）需扣减用户积分
- *使用抵扣*：预订确认弹窗加载用户已领取且未使用的优惠券（`isUsed = false` + `validFrom ≤ now ≤ validTo` + `minSpend ≤ totalPrice`），用户手动选择一张券，前端计算优惠后金额展示
- *生命周期*：过期优惠券（`now > validTo`）前端标记为"已失效"且不可领取，不自动删除（保留用户领取记录便于追溯）
...
*积分兑换事务处理*：后端服务端校验优惠券有效期后，若该券 `points > 0`，则启动 `prisma.$transaction` 事务（#flink("app/api/coupons/[id]/claim/route.ts#L37-L60")）：① 查询用户当前积分余额，积分不足则抛出 `INSUFFICIENT_POINTS` 异常并返回 400；② 通过 `user.update({ data: { points: { decrement: coupon.points } } })` 原子扣减积分；③ 写入 `UserCoupon` 关联表发放优惠券。三步操作在同一事务内原子完成，保证积分扣减与优惠券发放的强一致性。若 `points = 0`，则直接写入 `UserCoupon` 表，Prisma 复合主键 `@@id([userId, couponId])` 保证幂等，重复领取返回 400。

=== 积分兑换事务流程

当用户点击"积分兑换"按钮领取优惠券时，系统通过数据库事务（#flink("app/api/coupons/[id]/claim/route.ts#L37-L60")）保证积分扣减与优惠券发放的原子性：
...
#figure(
  diagram(
    ...
    node((0,1), [检查登录态\nstorage.isAuthenticated()],
      fill: rgb("#1e3a5f"), corner-radius: 3pt, width: 120pt),
    node((0,2), [POST /api/coupons/\[id\]/claim],
      fill: rgb("#3d2b00"), corner-radius: 3pt, width: 120pt),
    node((0,3), [后端校验优惠券有效期],
      fill: rgb("#2b1a4a"), corner-radius: 3pt, width: 120pt),
    node((0,4), [检查 coupon.points > 0?],
      fill: rgb("#2b1a4a"), corner-radius: 3pt, width: 120pt),

    // 免费券分支
    node((2,5), [直接写入 UserCoupon 表],
      fill: rgb("#1a4a2e"), corner-radius: 3pt, width: 110pt),
    node((2,6), [返回 201 成功],
      fill: rgb("#1e3a5f"), corner-radius: 3pt, width: 100pt),

    // 积分券分支（事务）
    node((-2,5), [prisma.\$transaction 开始],
      fill: rgb("#1a4a2e"), corner-radius: 3pt, width: 120pt),
    node((-2,6), [① 查询 user.points 余额],
      fill: rgb("#1a4a2e"), corner-radius: 3pt, width: 120pt),
    node((-2,7), [② 余额充足?],
      fill: rgb("#1a4a2e"), corner-radius: 3pt, width: 120pt),
    node((-2,8), [③ user.update\npoints decrement],
      fill: rgb("#1a4a2e"), corner-radius: 3pt, width: 120pt),
    node((-2,9), [④ 创建 UserCoupon 记录],
      fill: rgb("#1a4a2e"), corner-radius: 3pt, width: 120pt),
    node((-2,10), [事务提交],
      fill: rgb("#1a4a2e"), corner-radius: 3pt, width: 120pt),
    node((-2,11), [返回 201：兑换成功],
      fill: rgb("#1e3a5f"), corner-radius: 3pt, width: 120pt),

    // 错误分支
    node((0,8), [积分不足 → 返回 400],
      fill: rgb("#4a1a1a"), corner-radius: 3pt, width: 110pt),
    node((0,9), [已领取 P2002 → 返回 400],
      fill: rgb("#4a1a1a"), corner-radius: 3pt, width: 120pt),

    edge((0,0), (0,1), "->", stroke: white.darken(30%)),
    edge((0,1), (0,2), "->", stroke: white.darken(30%),
      label: text(size: 7pt)[已登录],
      label-side: left),
    edge((0,2), (0,3), "->", stroke: white.darken(30%)),
    edge((0,3), (0,4), "->", stroke: white.darken(30%)),
    edge((0,4), (2,5), "->", stroke: white.darken(30%),
      label: text(size: 7pt)[免费券],
      label-side: right),
    edge((2,5), (2,6), "->", stroke: white.darken(30%)),
    edge((0,4), (-2,5), "->", stroke: white.darken(30%),
      label: text(size: 7pt)[积分券],
      label-side: left),
    edge((-2,5), (-2,6), "->", stroke: white.darken(30%)),
    edge((-2,6), (-2,7), "->", stroke: white.darken(30%)),
    edge((-2,7), (-2,8), "->", stroke: white.darken(30%),
      label: text(size: 7pt)[充足],
      label-side: left),
    edge((-2,8), (-2,9), "->", stroke: white.darken(30%)),
    edge((-2,9), (-2,10), "->", stroke: white.darken(30%)),
    edge((-2,10), (-2,11), "->", stroke: white.darken(30%)),
    edge((-2,7), (0,8), "->", stroke: rgb("#ff6b6b"),
      label: text(size: 7pt)[不足],
      label-side: center),
    edge((2,5), (0,9), "->", stroke: rgb("#ff6b6b"),
      label: text(size: 7pt)[重复],
      label-side: center),
  ),
  caption: [积分兑换优惠券事务流程图]
)
...
- 页面：#flink("trip_front_end_taro/src/pages/myCoupons/index.jsx")
- 数据接口：#flink("trip_front_end_taro/src/services/coupon.js")（`getUserCoupons`）

=== 做了什么

展示当前用户已领取的全部优惠券，每张卡片显示名称、描述、折扣金额、最低消费门槛和有效期，按状态区分*可用*、*已使用*和*已过期*三种样式。可用券提供"去使用"按钮跳转酒店列表页。

=== 怎么做的

页面挂载时调用 `getUserCoupons()` 获取用户优惠券列表（返回 `UserCoupon` 记录，内联 `coupon` 对象）。每张卡片通过两个条件判断状态：`dayjs(item.validTo).isBefore(dayjs())` 判断是否过期，`userCoupon.status === 'USED'` 判断是否已使用。三种状态对应不同样式——可用券为正常配色 + 蓝色"去使用"按钮（点击后 `Taro.switchTab` 跳转酒店列表页），已过期和已使用券整体灰化（`coupon-card-expired` class）+ 禁用态按钮文字。卡片布局采用左右分栏：左侧展示名称、描述、标签（优惠券/已过期/已使用）和有效期，右侧展示折扣金额和消费门槛。空列表展示"暂无优惠券"提示。
...
`hotelMap` 页面使用 Taro `<Map>` 组件，支持"单酒店详情"和"搜索列表"两种模式（通过路由参数 `hotelId` 区分）。组件挂载时调用 `Taro.getLocation({ type: 'gcj02' })`（#flink("trip_front_end_taro/src/pages/hotelMap/index.jsx#L41-L51")）获取用户坐标。单酒店模式直接以传入坐标渲染 marker；列表模式通过 `getHotels()` 加载多酒店并以价格气泡标注。"导航"按钮触发 `Taro.openLocation()` 拉起微信内置地图导航，坐标统一 GCJ-02 无需转换。

#text(size: 8.5pt, fill: black.lighten(55%))[来源：#flink("trip_front_end_taro/src/pages/hotelMap/index.jsx#L147-L165")]
```jsx
// 导航到酒店——调起微信内置地图
const handleNavigate = () => {
  ...
  Taro.openLocation({ latitude: lat, longitude: lng, name, address, scale: 16 });
};
```
...
#table(
  ...
  [GET], [`/users?myStaff=true`], [MERCHANT], [—], [商户查询自己的职员列表（`merchantId = 当前用户ID`）],
  [GET], [`/users?role=X`], [公开], [Query: `role`], [按角色筛选用户基本信息（id/name/email），用于注册页选择商户],
  [GET], [`/users/profile`], [AUTH], [—], [当前登录用户资料，包含 `id/email/name/phone/role/points` 等字段；由 `verifyAuth()` 保护],
  [PUT], [`/users/[id]`], [AUTH], [Body: `name?` `phone?` `oldPassword?` `newPassword?`], [更新用户信息或修改密码；需为本人、ADMIN 或商户管理自己的职员（`targetUser.merchantId === currentUserId`）；密码修改仅限本人，需 `bcrypt.compare` 验证旧密码],
  [DELETE], [`/users/[id]`], [AUTH], [—], [删除用户，需为本人、ADMIN 或商户删除自己的职员],
...
评论与评价为两个独立模块：评价（Review）基于 `bookingId` 一单一评，评论（Comment）直接关联 `hotelId` 支持自由评论。创建和删除评论时自动调用 `updateHotelScore()` 重算酒店综合评分。

#table(
  columns: (auto, auto, auto, 1fr, 1fr),
  align: (center, left, center, left, left),
  table.header([*方法*], [*路径*], [*权限*], [*参数*], [*说明*]),
  [GET], [`/comments`], [公开], [Query: `hotelId?`、`userId?`], [评论列表，含 user(id,name) 和 hotel(id,nameZh)，按创建时间降序],
  [POST], [`/comments`], [AUTH], [Body: `hotelId`（必填）、`content`（必填）、`score?`（评分）], [创建评论，自动调用 `updateHotelScore()` 更新酒店评分],
  [GET], [`/comments/[id]`], [公开], [—], [单条评论详情，含 user 和 hotel 信息],
  [PUT], [`/comments/[id]`], [AUTH], [Body: `content?`、`score?`], [仅评论作者可修改，修改后自动更新酒店评分],
  [DELETE], [`/comments/[id]`], [AUTH], [—], [评论作者或 ADMIN 可删除，删除后自动更新酒店评分],
)
...
#text(size: 8.5pt, fill: black.lighten(55%))[来源：#flink("app/api/coupons/[id]/claim/route.ts#L36-L60")]
```ts
// 积分兑换：事务保证原子性
if (coupon.points > 0) {
  return await prisma.$transaction(async (tx) => {
    ...
    if (!user || user.points < coupon.points) {
      throw new Error('INSUFFICIENT_POINTS');  // 触发事务回滚
    }
    // 2. 原子扣减积分（Prisma decrement 保证并发安全）
...
// 免费券：直接写入，@@id([userId, couponId]) 复合主键防重复
const userCoupon = await prisma.userCoupon.create({ data: { userId, couponId } });
```

== 城市与标签接口 `/api/locations` 和 `/api/tags`
...
#table(
  ...
  // Locations 接口
  [GET], [`/locations`], [公开], [Query: `name?`（城市名搜索）、`type?`（domestic/overseas/province）], [返回城市列表，支持按名称模糊搜索和类型筛选；含 `parentId` 字段表示层级关系（省-市结构）],
  [POST], [`/locations`], [ADMIN], [Body: `name`、`description?`、`type`、`parentId?`], [管理员创建城市/省份；`type="province"` 为省级，`type="domestic"` 为国内城市（需 `parentId` 关联省份），`type="overseas"` 为海外城市],
  [GET], [`/locations/[id]`], [公开], [—], [单个城市详情，含关联的酒店数量],
  [PUT], [`/locations/[id]`], [ADMIN], [Body: 任意 location 字段], [更新城市信息],
  [DELETE], [`/locations/[id]`], [ADMIN], [—], [删除城市；若有酒店关联会返回 400 阻止删除],

  // Tags 接口
  [GET], [`/tags`], [公开], [—], [返回所有标签列表（含早餐、WiFi、停车场等）],
  [POST], [`/tags`], [ADMIN], [Body: `name`], [管理员创建标签，`name` 全局唯一],
  [PUT], [`/tags/[id]`], [ADMIN], [Body: `name`], [更新标签名称],
  [DELETE], [`/tags/[id]`], [ADMIN], [—], [删除标签；会自动清理 `hotel_tags` 关联记录],
)
...
#table(
  ...
  [GET], [`/room-availability/[id]`], [公开], [—], [查询单条库存记录（按 RoomAvailability.id），返回 `roomTypeId`、`date`、`quota`、`booked`、`price`],
  [PUT], [`/room-availability/[id]`], [MERCHANT], [Body: `quota?`、`price?`], [商户更新单日库存配额或动态价格],

  [GET], [`/map/geocoder`], [公开], [Query: `address`（地址字符串）], [腾讯地图地址解析接口，返回 GCJ-02 坐标 `{ lat, lng }`；用于酒店录入时根据地址自动填充经纬度],

  [GET], [`/swagger`], [公开], [—], [返回 OpenAPI JSON 规范，供 `/api-doc` 页面的 Swagger UI 渲染使用],
)
...
系统通过 #flink("app/lib/error-handler.ts") 统一处理所有 API 错误响应。核心函数 `handleApiError()`（#flink("app/lib/error-handler.ts#L129-L168")）自动识别 Prisma 数据库错误、Zod 校验错误和通用异常，返回标准化 JSON 格式。`ERROR_MESSAGES` 常量表（#flink("app/lib/error-handler.ts#L34-L55")）定义了 15+ 条用户友好的中文错误提示，避免向前端暴露技术细节。
...
#table(
  ...
  table.header([*状态码*], [*分类*], [*含义*], [*常见场景示例*]),

  [200], [成功], [GET 请求成功返回数据], [获取酒店列表、查询单个酒店详情],
  [201], [成功], [POST 创建资源成功], [创建预订、领取优惠券、提交评价],
  [204], [成功], [DELETE 删除成功（无响应体）], [删除收藏、取消预订],

  [400], [客户端错误], [请求参数错误或业务规则校验失败], [`积分不足`、`优惠券已领取`、`房间已售罄`、`日期格式错误`],
  [401], [客户端错误], [未登录或 Token 失效], [`Token 已过期，请重新登录`],
  [403], [客户端错误], [无权限执行操作], [`当前角色无 HOTEL_AUDIT 权限`],
  [404], [客户端错误], [资源不存在], [`酒店 ID 不存在`、`预订记录未找到`],
  [409], [客户端错误], [资源冲突], [`该邮箱已注册`（Prisma P2002 unique 冲突）],

  [500], [服务端错误], [服务器内部错误], [数据库连接失败、未捕获的异常],
  [503], [服务端错误], [服务暂不可用], [AI 推荐接口超时、第三方地图服务故障],
)
...
#table(
  ...
  align: (left, left, left),
  table.header([*错误信息*], [*触发场景*], [*对应文件*]),

  [`积分不足`], [用户积分 < 优惠券所需积分], [#flink("app/api/coupons/[id]/claim/route.ts#L44-L46")],
  [`优惠券已领取`], [UserCoupon 表复合主键冲突（P2002）], [#flink("app/api/coupons/[id]/claim/route.ts#L73-L75")],
  [`优惠券已过期`], [当前日期不在 validFrom~validTo 范围], [#flink("app/api/coupons/[id]/claim/route.ts#L31-L33")],
  [`房间已售罄`], [RoomAvailability.booked >= quota], [#flink("app/api/bookings/route.tsx#L208")],
  [`房间已关闭`], [RoomAvailability.isClosed = true], [#flink("app/api/bookings/route.tsx#L208")],
  [`邮箱已注册`], [User.email 唯一约束冲突（P2002）], [#flink("app/api/auth/register/route.tsx")],
  [`密码错误`], [bcrypt.compare 返回 false], [#flink("app/api/auth/login/route.tsx#L127")],
  [`用户不存在`], [prisma.user.findUnique 返回 null], [#flink("app/api/auth/login/route.tsx")],
  [`Token 已过期`], [jwt.verify 抛出 TokenExpiredError], [#flink("app/api/utils/auth.ts#L25")],
  [`无权限`], [Permission 表查询结果为空], [#flink("app/api/utils/permissions.ts#L110-L142")],
)

*错误处理工具链*：API route 文件的 `catch` 块通过一行 `handleApiError(error)` 调用自动适配上述全部错误类型（#flink("app/lib/error-handler.ts#L129")）——Prisma P2002 自动映射为"已存在"提示（#flink("app/lib/error-handler.ts#L64-L73")），Zod 校验失败自动收集全部字段错误（#flink("app/lib/error-handler.ts#L92-L108")），未知异常返回通用 500 错误。
...
- *统一图标*：借鉴 Phosphor Icons 开源库的 SVG path data（#flink("trip_front_end_taro/src/utils/icons.js#L108-L119") getIconUri 函数生成 data URI），封装 `Icon` 组件通过 data URI 渲染，40+ 处调用统一风格，深色模式经 `tokens` 自动适配；
...
+ *AI 自然语言酒店推荐*：用户以口语描述需求（如"找北京带早餐的五星级酒店"），系统两阶段处理：① 用 LLM（temperature=0）提取结构化搜索条件（目的地/价格/关键词），② 结合实时库存数据以流式输出（temperature=0.7）逐字推荐（#flink("app/api/ai/recommend/route.ts#L42")），降低传统筛选表单的操作成本。*技术成本*：GPT-4o API 调用费用约 \$2.50 每百万 tokens，平均每次推荐成本 <\$0.002。
...
+ *酒店列表地图视图切换*：列表页支持"列表模式 ↔ 地图模式"无缝切换，地图模式以价格气泡（`callout.content`）标注各酒店位置（#flink("trip_front_end_taro/src/pages/hotelMap/index.jsx#L115-L130")），点击气泡展示酒店缩略卡片，可直接预订。

+ *优惠券积分兑换全链路系统*：优惠券分为免费领取（`points = 0`）和积分兑换（`points > 0`）两种模式。免费券直接写入 `UserCoupon` 表；积分券通过 `prisma.$transaction` 事务保证原子性（#flink("app/api/coupons/[id]/claim/route.ts#L36-L60")）——事务内三步：① `tx.user.findUnique` 查询余额 → ② `tx.user.update({ points: { decrement } })` 原子扣减 → ③ `tx.userCoupon.create` 发放券。余额不足抛 `INSUFFICIENT_POINTS` 回滚整个事务。前端 `Promise.allSettled` 并发加载全量券和已领记录，`Set<claimedIds>` O(1) 去重判定；下单时在预订确认弹窗选择可用券自动抵扣，后端 `@@id([userId, couponId])` 复合主键幂等保护。

+ *搜索历史去重持久化*：首页关键词搜索历史最多保留 10 条，`unshift` 插入后 `filter` 过滤重复项（#flink("trip_front_end_taro/src/pages/home/index.jsx#L198-L210")），`Taro.setStorageSync` 跨会话持久化；历史条目一键回填搜索框，热门城市快捷入口降低输入成本。
...
+ *A-Z 拼音城市索引*：自建工具模块 #flink("trip_front_end_taro/src/utils/pinyinMap.js")，维护 70+ 常用汉字首字→拼音大写字母映射表（`CHAR_TO_INITIAL`），`groupByInitial()` 将全部城市按拼音首字母 A-Z 分组排序；城市选择弹窗右侧渲染字母索引栏，点击字母通过 `ScrollView.scrollIntoView` 快速定位对应分组，实现通讯录式城市检索体验。首页与列表页共享同一套组件逻辑与样式。

+ *搜索条件互斥与自动刷新*：关键词搜索与城市筛选设计为互斥关系——输入关键词时自动清除已选城市（全局搜索），选择城市时自动清空搜索关键词（定向筛选）；搜索框清空后 300ms 防抖自动重新加载全部结果，无需手动按回车，降低操作步骤。两个页面通过 `Taro.setStorageSync` 同步选择状态，保证跨页面一致性。

+ *列表页 N+1 查询消除*：后端 `GET /api/hotels` 接口通过 Prisma `include: { roomTypes: { select: { price: true } } }` 一次性返回所有酒店及其全部房型价格（单次 SQL JOIN），前端在内存中计算每个酒店的 `Math.min(roomTypes.map(rt => rt.price))`，彻底消除早期"每个酒店单独请求最低价"的 N+1 查询问题。10 条列表数据的响应延迟从约 500ms 降至约 150ms，数据库查询次数从 11 次降至 1 次。

+ *详情页并行化加载*：详情页需加载酒店基本信息、房型库存、收藏状态、评价列表四类数据，早期串行请求总耗时约 800ms。通过 `Promise.all([getHotelById(), getRoomTypes(), checkFavorite(), getComments()])` 并行发起，总耗时降至约 250ms（取决于最慢单个请求），首屏可交互时间（TTI）缩短约 70%，用户感知速度显著提升。单个接口失败时通过 `.catch(() => null)` 降级处理，不影响其他数据展示。
...
项目在代码质量优化中抽象出三个核心工具模块，消除重复逻辑并提升可维护性：

- #flink("app/constants/index.ts") — 统一常量定义，包含 API 端点（`API_ENDPOINTS`）、状态码（`HOTEL_STATUS`、`BOOKING_STATUS`）、角色名（`ROLES`）、存储键（`STORAGE_KEYS`）等。所有魔法字符串替换为语义化常量，修改 API 路径只需更新一处即可全局生效，减少字符串拼写错误；

- #flink("app/lib/auth-storage.ts#L18-L101") — 认证状态统一存储管理，封装所有 localStorage 操作为类型安全的 API。核心方法：`getToken()`（#flink("app/lib/auth-storage.ts#L18")）、`saveAuth(token, refreshToken, user)`（#flink("app/lib/auth-storage.ts#L79")）批量写入认证三元组、`clearAuth()`（#flink("app/lib/auth-storage.ts#L89")）一键清除、`isAuthenticated()`（#flink("app/lib/auth-storage.ts#L100")）检查登录态。自动处理 SSR 环境判断（`typeof window !== 'undefined'`）、JSON 序列化、角色校验，被约 8 个文件共享；

- #flink("app/lib/error-handler.ts#L129-L168") — 标准化错误处理工具，`handleApiError()` 函数（#flink("app/lib/error-handler.ts#L129")）统一处理三类异常：Prisma 数据库错误（`PrismaClientKnownRequestError`，#flink("app/lib/error-handler.ts#L60-L87") 含 P2002/P2025 等 6 种错误码映射）、Zod 校验错误（`ZodError`，#flink("app/lib/error-handler.ts#L92-L108") 自动收集全部字段错误）和通用异常。`ERROR_MESSAGES` 常量表（#flink("app/lib/error-handler.ts#L34-L55")）含 15+ 条用户友好的中文错误提示，API route 文件通过一行调用即可返回规范化错误响应：

#text(size: 8.5pt, fill: black.lighten(55%))[来源：#flink("app/lib/error-handler.ts#L129-L168")]
```ts
// 统一错误处理函数——一行代码替代各 API 文件的重复 catch 块
export function handleApiError(error: unknown, defaultMessage: string): ErrorResponse {
  if (error instanceof PrismaClientKnownRequestError) return handlePrismaError(error);
  if (error instanceof ZodError) return handleZodError(error);
  if (error instanceof Error) { /* 检查已知业务错误 */ }
...
- *TypeScript 全链路*：后端 Next.js API Routes、前端 Web 页面、Service 层全部使用 TypeScript，Zod 在 API 边界做运行时校验，Prisma Client 提供数据库操作类型安全。后端 API 路由中所有动态构建的查询条件均定义了具体 TypeScript 接口（如 `HotelWhereInput`、`PriceFilter`），前端组件定义 `GuestInfo`、`UploadResponse`、`RoomFormItem` 等业务接口，消除 18 处 `any` 类型（后端 13 处 + 前端 5 处），确保编译期类型检查覆盖；
- *命名约定*：JS/TS 变量和函数使用 camelCase，React 组件使用 PascalCase，数据库列名使用 camelCase（Prisma 惯例），CSS class 使用 kebab-case；
- *Service 层代码清理*：小程序端 7 个 service 文件（#flink("trip_front_end_taro/src/services/auth.js")、#flink("trip_front_end_taro/src/services/booking.js")、#flink("trip_front_end_taro/src/services/hotel.js") 等）共清理约 52 处冗余 `console.log` 语句，统一由 #flink("trip_front_end_taro/src/services/request.js#L15-L25") 的请求拦截器集中记录；清理 #flink("trip_front_end_taro/src/services/comments.js") 中重复的 review 数据包装函数，单一职责原则下每个文件仅保留核心业务方法；
- *README 完备性*：项目根目录 `README.md` 包含：项目简介、技术栈、本地安装步骤、环境变量说明、数据库初始化命令（`prisma db push` + `db:seed`）、测试账号列表、常见问题排查，新成员可在 10 分钟内完成本地启动；
- *错误边界机制*：Next.js App Router 的 #flink("app/error.tsx") / #flink("app/loading.tsx") 和 #flink("app/merchant/error.tsx") / #flink("app/merchant/loading.tsx") 在全局和商户端实现统一错误处理和加载状态，防止白屏，改善用户体验；错误页面使用 Ant Design `Result` 组件展示友好提示，提供"重试"和"返回"按钮，加载状态使用 `Spin` 组件（已升级 `description` 属性替代废弃的 `tip`）。
...
#table(
  ...
  [`SearchSuggestion`], [首页搜索建议], [展示搜索历史（最多 10 条，去重）和热门城市快捷入口；历史记录通过 `Taro Storage` 持久化，`unshift` 新搜索词并过滤重复项],
  [`TencentMapSelector`], [商户酒店录入页], [Web 端腾讯地图坐标拾取组件，集成地图 SDK，支持地址搜索、标记拖拽、坐标回填表单],
  [`Navbar`], [Web 端全局导航], [含主题切换按钮（调用 `useTheme().setTheme()`）、登录/注册入口、用户头像下拉菜单],
  [`Footer`], [Web 端页脚], [展示版权信息、友情链接、ICP 备案号],
  [`FeatureItem`], [Web 端首页特性卡片], [首页"系统特点"区块的卡片组件，接收 `icon`、`title`、`description` props],
  [`HotelDetailsDrawer`], [管理员酒店审核页详情展示], [酒店详情抽屉组件，展示酒店基本信息、设施、图片、房型列表；通过 Ant Design Drawer + Descriptions 布局，图片支持预览组；从 `app/admin/hotels/page.tsx` 抽离（455行优化至222行）],
  [`HotelCommentsDrawer`], [管理员酒店审核页评论管理], [酒店评论抽屉组件，展示评论列表和删除操作；含 Popconfirm 二次确认、乐观UI更新（删除后立即从列表移除）],
  [`useHotelTableColumns`], [管理员酒店审核页表格列定义], [自定义 Hook，集中管理表格列配置和操作按钮（查看、评论、下线、驳回、上线）；接收回调函数（`onView`、`onViewComments`、`onStatusChange`），返回 Ant Design Table 的 `columns` 配置；实现状态驱动的按钮显示逻辑（published 时显示下线/驳回按钮，offline 时显示上线按钮）],
)
...
- #flink("app/lib/status-utils.ts") — Web 端预订状态工具，提供 `getStatusColor()` / `getStatusText()` 统一映射（`BookingStatus` → Ant Design Tag 颜色/中文文本），消除商户酒店管理、预订管理、仪表盘 3 个页面的重复代码；
- #flink("trip_front_end_taro/src/utils/status.js") — Taro 端订单状态工具，提供 `getStatusText()` / `getStatusColor(status, tokens)` 动态颜色映射（支持深色模式），消除订单列表、订单详情 2 个页面的重复代码；
- `app/constants/index.ts` — 统一常量定义（API端点、状态码、角色名、存储键），所有Service层和页面组件import同一来源，魔法字符串零重复；
- `app/lib/auth-storage.ts` — 认证状态管理单例，封装Token/User的读写、角色校验、SSR环境守卫，被约 8 个文件共享；
- `app/lib/error-handler.ts` — 错误处理工具，统一处理Prisma错误、Zod错误和通用异常，提供用户友好的中文错误消息映射表；
- `app/api/utils/permissions.ts` — 权限检查工具，实现15分钟TTL缓存机制（`PermissionCache` 类），提供单权限检查（`checkPermission`）和批量检查（`checkAnyPermission`、`checkAllPermissions`）API，性能提升95%+。

= 架构决策记录

本章记录项目关键技术选型的决策理由与权衡，便于评审和后续维护参考。

== ADR-1：Next.js App Router 全栈架构

*背景*：需要同时支持 Web 管理端和 REST API，团队规模小（3人），希望减少技术栈碎片化。

*决策*：采用 Next.js 14 App Router，前后端同仓库、同一个 `app/` 目录结构。API Routes 直接作为后端服务，无需独立 Express/Koa 服务。

*权衡*：放弃了前后端分离部署的灵活性，换取统一部署、类型共享（`app/types/index.ts`）和更低运维成本。对于学生项目规模（14 张表、~30 个 API），单体架构足够。

== ADR-2：Prisma ORM 替代原生 SQL

*背景*：团队需要快速建表、类型安全的数据库操作，且频繁修改 Schema。

*决策*：使用 Prisma 6 + `prisma/schema.prisma` 声明式 Schema，`prisma db push` 同步数据库结构，Client 自动生成全套 TypeScript 类型。

*权衡*：牺牲复杂查询的灵活性（如窗口函数、递归 CTE），换取开发速度和类型安全。实际项目中所有查询均可通过 Prisma Client API 表达（含 `$transaction`、嵌套 `include`、聚合 `_count`），未遇到需要 raw SQL 的场景。

== ADR-3：JWT 双 Token 认证机制

*背景*：小程序和 Web 端均需无状态认证，且用户体验要求登录态持久化。

*决策*：Access Token（1h 有效期）+ Refresh Token（7d 有效期）双 Token 方案。Access Token 过期后前端拦截器自动以 Refresh Token 换取新 Token（#flink("app/lib/request.ts#L47-L145")），请求队列排队等待刷新完成后重发。

*权衡*：比 Session 方案实现复杂度更高（需处理并发刷新、队列机制），但天然支持多端登录和横向扩展（无需共享 Session 存储）。

== ADR-4：Taro 跨端方案替代原生开发

*背景*：需支持微信小程序为主、兼顾其他平台可能性，团队熟悉 React。

*决策*：Taro 4 编译型方案，React JSX 编写，编译至微信/支付宝/H5 等多平台。

*权衡*：相比原生 WXML 性能略有损耗，但一套代码多端复用节省约 50% 工作量。Taro 对微信原生 API（`getLocation`、`openLocation`、`Map` 组件）的封装兼容性良好，未遇到需要降级为原生写法的场景。

== ADR-5：LangChain + Zod 结构化输出

*背景*：AI 推荐功能需从用户自然语言中提取结构化搜索条件，再基于数据库结果生成推荐文案。

*决策*：LangChain `withStructuredOutput(zodSchema)` 强制 LLM 输出符合 Zod Schema 的 JSON（#flink("app/api/ai/recommend/route.ts#L42")），第一阶段 temperature=0 确保提取准确性，第二阶段 temperature=0.7 兼顾创意。

*权衡*：依赖 OpenAI API（单次调用成本约 \$0.002），但两阶段分离保证了意图提取的确定性和推荐文案的多样性。Zod Schema 复用于 API 校验和 LLM 输出约束，减少重复定义。

= 安全设计总结

== 认证安全

- *密码存储*：bcrypt salt=10 哈希，数据库不存储明文密码（#flink("app/api/auth/login/route.tsx#L3") `bcrypt.compare`）
- *JWT 密钥守护*：服务启动时强制校验 `JWT_SECRET` 非默认值，缺失或使用占位符直接抛出启动异常阻断服务（#flink("app/api/utils/auth.ts#L4-L7")）
- *Token 双层机制*：Access Token 短期（1h）限制泄露窗口，Refresh Token 长期（7d）保持登录体验；前端拦截器自动刷新，用户无感知

== 接口安全

- *登录速率限制*：内存级 IP 频率限制器，同一 IP 15 分钟内最多 5 次登录尝试（#flink("app/api/auth/login/route.tsx#L7-L24")），超限返回 429 + `Retry-After` 头
- *权限隔离*：`verifyAuth()`（#flink("app/api/utils/auth.ts#L17-L30")）统一鉴权，~29 个 API route 共享；商户只能操作自有酒店，管理员通过 `checkPermission()`（#flink("app/api/utils/permissions.ts#L128-L143")）细粒度控制。所有权限检查统一使用 `checkPermission()` 函数替代各 API 路由中重复的 `prisma.user.findUnique({ include: { role: { include: { rolePermission } } } })` 查询（#flink("app/api/users/route.tsx")、#flink("app/api/users/[id]/route.tsx")、#flink("app/api/hotels/[id]/route.tsx")、#flink("app/api/room-types/[id]/route.tsx") 共 5 处替换），复用 15 分钟 TTL 缓存避免重复数据库查询
- *Zod 边界校验*：所有 API 入参经 Zod Schema 运行时校验（如 #flink("app/api/bookings/route.tsx#L7-L15")），非法数据在进入业务逻辑前即被拦截

== 数据安全

- *事务原子性*：涉及资金/积分操作使用 `prisma.$transaction` 保证一致性（预订扣库存、积分兑券），异常自动回滚
- *幂等保护*：复合主键（`UserCoupon @@id([userId, couponId])`）和唯一约束（`Review bookingId @unique`、`RoomAvailability @@unique([roomTypeId, date])`）从数据层防重复
- *审计追踪*：`HotelAuditLog` 表追加写入（不更新/删除），记录酒店状态变更操作者和时间

== 前端安全

- *Token 安全存储*：Web 端使用 `localStorage`（#flink("app/lib/auth-storage.ts#L18")），小程序端使用 `Taro.getStorageSync`，均封装为统一 API 避免直接操作
- *请求拦截*：axios 请求拦截器自动注入 `Authorization` Header（#flink("app/lib/request.ts#L32-L44")），响应拦截器处理 401 自动刷新 Token 或跳转登录
- *并发刷新控制*：`isRefreshing` 锁 + `failedQueue` 队列（#flink("app/lib/request.ts#L12-L16")）确保多个并发 401 请求只触发一次 Token 刷新，其余排队等待

= 后端查询性能优化

== API 响应字段精简

*核心文件*：#flink("app/api/hotels/route.tsx")（酒店列表）、#flink("app/api/favorites/route.tsx")（收藏列表）

*做了什么*：酒店列表和收藏列表接口原先使用 `include: { roomTypes: true }` 返回完整房型对象（含 description、amenities、images、stock、availability 等全部字段），移动端列表页仅需 `id` 和 `price` 两个字段。

*怎么做的*：将 `include` 改为 `select` 精确指定返回字段：

```ts
// 优化前：返回完整 roomType 对象（~20 字段 × N 条房型）
include: { roomTypes: true }

// 优化后：仅返回列表页所需字段
roomTypes: { select: { id: true, price: true } }
```

同样优化了 `location`（仅返回 `id/name/type`）、`merchant`（仅返回 `id/name`）、`hotelTags`（仅返回 `tag.id/tag.name`）的关联查询。

*性能收益*：单次 API 响应数据量减少约 40-50%，对移动端弱网环境尤为显著。

== 预订事务批量查询优化

*核心文件*：#flink("app/api/bookings/route.tsx#L244-L330")

*做了什么*：预订接口创建订单时需要检查入住区间内每一天的房间可用性。原实现对每一天执行 3 次数据库操作（`upsert` + `findUnique` + `updateMany`），7 晚预订共 21 次数据库交互。

*怎么做的*：将查询逻辑重构为两阶段：

- *批量查询阶段*：`findMany` 一次性获取所有日期记录 → `createMany({ skipDuplicates })` 补齐缺失日期 → 再次 `findMany` 获取完整数据。共 3 次数据库操作替代原先 N×2 次查询。
- *逐日原子更新阶段*：仍使用 `updateMany({ where: { booked: { lt: quota } } })` 保证并发安全，但省去了每日的 `upsert` 和 `findUnique`。

*性能收益*：7 晚预订从 21 次降至 10 次数据库操作（减少 52%），预订接口响应时间降低约 30%。

== 类型安全加固

*涉及文件*：#flink("app/api/hotels/route.tsx")、#flink("app/api/hotels/[id]/route.tsx")、#flink("app/api/bookings/route.tsx")、#flink("app/api/bookings/[id]/route.tsx")、#flink("app/api/users/[id]/route.tsx")、#flink("app/api/tags/route.tsx")、#flink("app/api/locations/route.tsx")、#flink("app/api/comments/route.ts")、#flink("app/api/auth/register/route.tsx")

*做了什么*：消除后端 API 路由中全部 13 处 `any` 类型注解，替换为具体的 TypeScript 接口或内联类型。
...
interface HotelWhereInput {
  ...
  OR?: Array<{ nameZh?: { contains: string } }>;
}
const where: HotelWhereInput = {};
const priceFilter: PriceFilter = {};
```

*收益*：编译器可在开发阶段捕获字段拼写错误和类型不匹配，减少运行时异常风险。
