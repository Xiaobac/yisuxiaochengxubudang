...
列表体使用 `<ScrollView scrollY onScrollToLower={loadMoreHotels} lowerThreshold={100}>` 包裹，距底部 100px 时触发加载。后端 `GET /api/hotels` 支持 `page` + `limit` 分页参数，前端每页加载 10 条：

- `loadHotels()`：首次加载或条件变更时重置 `currentPage=1`，获取第一页数据并通过 `total` 字段判断是否还有更多；
- `loadMoreHotels()`：上滑时以 `currentPage+1` 请求下一页，将新数据追加到 `hotelList` 尾部，`loadingMore` 状态防止重复触发；
- 底部展示"加载更多..."或"已加载全部酒店"提示，用户感知明确。

这种「服务端分页 + 客户端追加」的方案避免了一次性加载全部酒店导致的渲染压力，尤其在数据量较大时可显著减少首屏等待时间。
