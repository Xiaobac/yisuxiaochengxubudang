...
#table(
  ...
  [编码规范与 README], [3], [ESLint 统一规范 ✓ \ TypeScript 全链路(消除全部13处any) ✓ \ README 含完整启动/种子/排障指南 ✓], [§ 代码质量],
...
*测试环境*：MacBook Pro M1 (16GB RAM) + MySQL 8.x 本地数据库 + 1000 条酒店测试数据 + 单机测试
...
- *精细化权限*：三角色（USER(STAFF) / MERCHANT / ADMIN）+ 细粒度权限（HOTEL_AUDIT / HOTEL_DELETE）双层鉴权，每次状态变更均写入审计日志；
...
页面使用 `useAuth` Hook 校验当前用户角色，非 ADMIN 自动跳转登录页。表格以原生 HTML `<table>` 渲染优惠券列表，每行展示 `code`、`points`（所需积分）、`name`、`discount`（折扣金额）+ `minSpend`（最低消费）、有效期范围，以及"编辑"/"删除"两个操作按钮。

新增和编辑共用同一个 Modal 弹窗表单（通过 `editingCoupon` 状态区分模式），表单字段包括：优惠券代码（`code`，必填）、名称（`name`，必填）、折扣金额（`discount`，必填）、最低消费（`minSpend`，可选）、兑换所需积分（`points`，可选，默认 0 表示免费领取）、开始/结束时间（`validFrom`/`validTo`，必填）、描述（`description`，可选）。提交时将字符串字段解析为数值类型后调用 `createCoupon` 或 `updateCoupon`，成功后乐观更新本地 state。删除操作通过 `window.confirm` 二次确认后调用 `deleteCoupon`，成功后从列表中移除。
...
- *TypeScript 全链路*：后端 Next.js API Routes、前端 Web 页面、Service 层全部使用 TypeScript，Zod 在 API 边界做运行时校验，Prisma Client 提供数据库操作类型安全。后端 API 路由中所有动态构建的查询条件均定义了具体 TypeScript 接口（如 `HotelWhereInput`、`PriceFilter`），消除 13 处 `any` 类型，确保编译期类型检查覆盖；
